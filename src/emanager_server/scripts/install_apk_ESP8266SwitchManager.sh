...
function waitNewDevice {
	...
	#the next conditional is another fix for hanged cpu monitor process detected for all device models but motorola_moto_g6 whose id is ZY322VVCKZ
	if [ "$LAST_DEVICE_ID" != "ZY322VVCKZ" ];then
        echo "Fixing cpu monitor process hanged"
        ./scripts/fix_hanged_cpu_monitor.sh $LAST_DEVICE_IP
	fi
}
